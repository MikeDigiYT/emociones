...
classifier = ml5.imageClassifier('https://teachablemachine.withgoogle.com/models/Snfe_1XZS/model.json',modelLoaded);
...
function gotResult(error, results) {
    if (error) {
        ...
        speak();
        if(results[0].label == "osito" )
        {
            document.getElementById("update_emoji").innerHTML = "&#128522";
        }
        if(results[0].label == "pulpito")
        {
        ...
        if(results[1].label == "osito")
        {
            document.getElementById("update_emoji2").innnerHTML = "&#128522;";
        }
        if(results[1].label == "pulpito")
        {
